...
4. `patient4` is a middle aged (`60`), female patient, with a higher BMI (`41`). What do you think?
...
shhs_data_filtered2 <- shhs_data %>% select(any_cvd, age_s1, gender, neck20) %>% 
#use drop_na() for now  
  drop_na()

visdat::vis_dat(shhs_data_filtered2)

#separate into test/train sets
shhs_train_index <- createDataPartition(shhs_data_filtered2$any_cvd, p=0.85, list=FALSE)


shhs_train_data2 <- shhs_data_filtered2[shhs_train_index,]
shhs_test_data2 <- shhs_data_filtered2[-shhs_train_index,]

basic_model2 <- glm(any_cvd ~ gender + age_s1 + neck20 ,
                    family = "binomial", 
                    #we put our data into the data argument
                    data =    shhs_train_data2
)

predictions_model2 <- augment(basic_model2, newdata=shhs_test_data2,  type.predict = "response")

predictions_model2 %>% ggplot(aes(x=.fitted)) + geom_histogram()
